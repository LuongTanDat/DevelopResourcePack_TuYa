...
const DOWNLOAD_CMD_S download_cmd[] =
{
  ...
  {DPID_COUNTDOWN, DP_TYPE_ENUM},
  {DPID_STOP_CURTAIN, DP_TYPE_BOOL},
};
...
void all_data_update(void)
{
  ...
  mcu_dp_bool_update(DPID_POWER,当前Power); //BOOL型数据上报;
  mcu_dp_enum_update(DPID_STATUS,当前Status); //枚举型数据上报;
  mcu_dp_fault_update(DPID_FAULT,当前Fault); //故障型数据上报;
  mcu_dp_enum_update(DPID_COUNTDOWN,当前Countdown); //枚举型数据上报;
  mcu_dp_bool_update(DPID_STOP_CURTAIN,当前stop); //BOOL型数据上报;
...
/*****************************************************************************
Function name : dp_download_power_handle
Function description : on DPID_POWER processing function
...
static unsigned char dp_download_countdown_handle(const unsigned char value[], unsigned short length)
{
  ...
  countdown = mcu_get_dp_download_enum(value,length);
  switch(countdown)
  {
    case 0:
  ...
  //处理完DP数据后应有反馈
  ret = mcu_dp_enum_update(DPID_COUNTDOWN,countdown);
  if(ret == SUCCESS)
    return SUCCESS;
  else
    return ERROR;
...
/*****************************************************************************
Function name : dp_download_stop_curtain_handle
Function description : on DPID_STOP_CURTAIN processing function
Input parameter : value:Source data
        : length:Data length
Return parameter : Successful return:SUCCESS/Failed to return:ERROR
Instructions for use : 可下发可上报type,need to report the result to App after data is dealt with
*****************************************************************************/
static unsigned char dp_download_stop_curtain_handle(const unsigned char value[], unsigned short length)
{
  //Example: The current DP type isBOOL
  unsigned char ret;
  //0:关/1:开
  unsigned char stop_curtain;
  
  stop_curtain = mcu_get_dp_download_bool(value,length);
  if(stop_curtain == 0)
  {
    //开关关
  }
  ...
  //处理完DP数据后应有反馈
  ret = mcu_dp_bool_update(DPID_STOP_CURTAIN,stop_curtain);
  if(ret == SUCCESS)
    return SUCCESS;
  else
    return ERROR;
  ...
  ***********************************/
  ...
  switch(dpid)
  {
    case DPID_POWER:
      ...
      break;
    case DPID_COUNTDOWN:
      //Countdownprocessing function
      ret = dp_download_countdown_handle(value,length);
      break;
    case DPID_STOP_CURTAIN:
      //stopprocessing function
      ret = dp_download_stop_curtain_handle(value,length);
      break;
